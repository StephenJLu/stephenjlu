...
import pubCover from '/docs/sjlu-pub-with-cover.pdf';
...
const ResearchText = ({ visible, titleId }: { visible: boolean; titleId: string }) => (
  ...
  <TableRow className={styles.row} data-visible={visible}>
    ...
    <TableCell>
      <List className={styles.description} data-visible={visible} size="l" as={'ul'}>
        <ListItem><Text className={styles.description} data-visible={visible} size="l">Reevaluation of the role of early trypsin activity in the transcriptional activation of the late trypsin gene in the mosquito <i>Aedes aegypti</i><br/><Link secondary href={pubCover} target="_blank" rel="noopener noreferrer" download>Download PDF</Link></Text></ListItem>
  ...
  <TableRow className={styles.row} data-visible={visible}>
    ...
    <TableCell>
      ...
      <Text className={styles.description} data-visible={visible} size={'l'} as={'p'}>Before his career in forensics, Stephen worked for six years as a published research scientist. Under Dr. Michael Wells at the University of Arizona, he studied protease regulation in the yellow fever mosquito, <i>Aedes aegypti</i>. He also <Link secondary href={'https://www.training.nih.gov/research-training/pb/sip/'}>completed an internship</Link> at the National Institutes of Health/NIDDK, studying neutral lipid trafficking in Niemann-Pick disease type C (NPC) under Dr. E. Joan Blanchette-Mackie.</Text>
      <Text className={styles.description} data-visible={visible} size={'l'} as={'p'}>Stephen volunteers his time and resources to support and improve the lives of people living with <Link secondary href={'/als'}>amyotrophic lateral sclerosis (ALS)</Link>. He served as Executive Director of A Lasting Strength, a non-profit organization dedicated to supporting patients living with ALS and mobility disabilities; he also served as a community member of UC San Diego Health's Patient and Family Advisory Council, working to unify patients, family, and team members to enhance the experience for everyone, evaluate strategies and improve quality and safety outcomes.</Text>
      <Text className={styles.description} data-visible={visible} size={'l'} as={'p'}>Stephen holds an Executive MBA with Honors from Quantic School of Business and Technology and a Bachelor of Science with Honors, <i>magna cum laude</i>, in Biochemistry and Molecular Biophysics and Molecular and Cellular Biology from the University of Arizona, where he was elected to Phi Beta Kappa. Stephen is a Society for Human Resource Management Certified Professional (SHRM-CP).</Text>
      <Text className={styles.description} data-visible={visible} size={'l'} as={'p'}>In his free time, Stephen enjoys reading and writing, web development, and electronic music composition and production.</Text>
  </>
